...
// The game boots underneath as usual, on whichever save slot is active; this layer decides when to press its "Enter" button.
// Three slots: rfgrowco-slot1..3. The active one is remembered in rfgrowco-slot. Switching slot reloads the page,
// because the game reads its save once, at boot.
(function () {
  'use strict';
  var SLOTS = 3;
  ...
  var urlSave = (function () { try { return new URLSearchParams(location.search).get('save'); } catch (e) { return null; } })();   // ?save=name is a developer slot outside the three
  function active() { var n = +(ls('rfgrowco-slot') || 1); return n >= 1 && n <= SLOTS ? n : 1; }
  function keyOf(n) { return 'rfgrowco-slot' + n; }
  ...
  function slotRow(n) {
    ...
    return '<div class="rf-slot' + (on ? ' on' : '') + '"><div class="rf-slot-info"><b>Slot ' + n + (on ? ' · loaded' : '') + '</b><span>' + (has ? 'Day ' + (s.day || 1) + ' · level ' + (s.level || 1) + ' · ' + money(s.bank) + ' · rep ' + Math.round(s.rep || 0) : 'Empty slot: starts with a tent, a seed and ' + money(220)) + '</span></div>' +
  ...
  function showMain() {
    card.classList.remove('wide'); var rows = ''; for (var n = 1; n <= SLOTS; n++) rows += slotRow(n);
